    ...
    <FadeIn className="min-h-screen bg-[#F9FAFB] flex relative">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <FadeIn className="flex flex-col gap-6 max-w-6xl mx-auto">
                  ...
                  <div className="flex flex-col h-full bg-white/95 backdrop-blur-xl shadow-2xl border-l border-[#E5E7EB]">
                    ...
                    {editModalOpen && (
                      <div className="fixed inset-0 z-50 flex items-center justify-center bg-white bg-opacity-40">
                        ...
                        >
                          <button type="button" className="absolute top-3 right-3 text-gray-400 hover:text-[#0D9488] text-2xl" onClick={() => setEditModalOpen(false)}>&times;</button>
                          ...
                          <div className="flex gap-3 mt-8">
                            <button type="button" className="flex-1 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-100" onClick={() => setEditModalOpen(false)}>Cancel</button>
                            <button type="submit" className="flex-1 py-2 rounded-lg bg-[#14B8A6] text-white font-semibold hover:bg-[#0D9488]">Save</button>
                          </div>
                        </form>
                      </div>
                    )}
                    ...
                    {deleteModalOpen && (
                      <div className="fixed inset-0 z-50 flex items-center justify-center bg-white bg-opacity-40">
                        <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md relative">
                          <button type="button" className="absolute top-3 right-3 text-gray-400 hover:text-[#DC2626] text-2xl" onClick={() => setDeleteModalOpen(false)}>&times;</button>
                          ...
                          <div className="flex gap-3 mt-8">
                            <button type="button" className="flex-1 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-100" onClick={() => setDeleteModalOpen(false)}>Cancel</button>
                            <button type="button" className="flex-1 py-2 rounded-lg bg-[#DC2626] text-white font-semibold hover:bg-[#B91C1C]" onClick={handleDeleteAccount}>Delete</button>
                          </div>
                        </div>
                      </div>
                    )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <SlideInRight>
              <button className="bg-gradient-to-r from-[#14B8A6] to-[#0D9488] text-white rounded-xl shadow p-4 flex flex-col items-center hover:from-[#0D9488] hover:to-[#14B8A6] transition w-full">
                  ...
                  <ul className="divide-y divide-[#E5E7EB]">
                    <li className="flex items-center justify-between py-2">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-[#22C55E] bg-[#D1FAE5] text-lg">SN</div>
                        <div>
                    ...
                    <li className="flex items-center justify-between py-2">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-[#F59E0B] bg-[#FEF3C7] text-lg">RS</div>
                        <div>
                    ...
                    <li className="flex items-center justify-between py-2">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-[#6366F1] bg-[#E0E7FF] text-lg">PN</div>
                        <div>
              ...
              <ScaleIn delay={0.1}>
                <section className="bg-[#FFFFFF] rounded-xl shadow p-6 border border-[#E5E7EB]">
                  ...
                  <ul className="space-y-2">
                    {[
                      { name: "Metformin", dose: "500mg · 1 tablet · Twice daily", status: "Active", color: "#22C55E", bg: "#D1FAE5" },
                      { name: "Atorvastatin", dose: "10mg · 1 tablet · Once daily", status: "Refill Due", color: "#F59E0B", bg: "#FEF3C7" },
                      { name: "Amlodipine", dose: "5mg · 1 tablet · Once daily", status: "Active", color: "#22C55E", bg: "#D1FAE5" },
                    ].map((rx) => (
                      <li key={rx.name} className="flex items-center gap-3 justify-between py-1">
                        <div className="flex items-center gap-2">
                          <span className="w-2 h-2 rounded-full inline-block" style={{ background: rx.color }}></span>
                          <span className="font-semibold text-[#111827]">{rx.name}</span>
                          <span className="text-xs text-[#6B7280]">{rx.dose}</span>
                        </div>
                        <span className="text-xs font-medium rounded px-2 py-0.5" style={{ color: rx.color, background: rx.bg }}>{rx.status}</span>
                      </li>
                    ))}
                  </ul>
                </section>
              ...
              <ScaleIn delay={0.2}>
                <section className="bg-[#FFFFFF] rounded-xl shadow p-6 border border-[#E5E7EB]">
                  ...
                  <div className="flex items-end gap-2 h-20">
                    {[60, 80, 70, 90, 60].map((h, i) => (
                      <div key={i} className="w-6 rounded-t-lg" style={{ height: `${h}%`, background: `rgba(20,184,166,${0.3 + i * 0.15})` }}></div>
                    ))}
                  </div>
                  <div className="flex justify-between text-xs text-[#9CA3AF] mt-2">
                    {["J", "F", "M", "A", "M"].map((m) => <span key={m}>{m}</span>)}
                  </div>
                </section>
              </ScaleIn>
            </FadeIn>
